...
class Demo:
    def __init__(self, parent=None):
        ...
        self.txt1.bind("<Return>", lambda event: self.search_student())
        ...
        columns = ["id", "name", "gender", "mobile", "email", "password", "status"]
        self.student_table = ttk.Treeview(self.table_frame, columns=columns, show='headings')

        for i in columns:
            self.student_table.heading(i, text=i.capitalize())
            self.student_table.column(i, anchor="center", width=120, minwidth=90)

        self.student_table_y_scroll = ttk.Scrollbar(self.table_frame, orient="vertical", command=self.student_table.yview)
        self.student_table_x_scroll = ttk.Scrollbar(self.table_frame, orient="horizontal", command=self.student_table.xview)
        self.student_table.configure(yscrollcommand=self.student_table_y_scroll.set, xscrollcommand=self.student_table_x_scroll.set)

        self.student_table.grid(row=0, column=0, sticky="nsew", pady=15, padx=(15, 0))
        self.student_table_y_scroll.grid(row=0, column=1, sticky="ns", pady=15)
        self.student_table_x_scroll.grid(row=1, column=0, sticky="ew", padx=(15, 0), pady=(0, 15))
    ...
    def update_table_data(self, rows):
        ...
        for index, row in enumerate(rows):
            self.student_table.insert('', values=row, index=index)
    ...
    def search_student(self):
        ...
        if not val:
            self.get_values()
            return
        q = "SELECT * FROM students WHERE LOWER(name) LIKE LOWER(%s)"
    ...
    def update(self):
        treeviewid = self.student_table.selection()
        if len(treeviewid) == 0:
            tkinter.messagebox.showerror("Error", "Please select row to update", parent=self.root)
            return

        self.item = self.student_table.item(treeviewid).get("values")
        print(self.item)
        ...
        # Status
        self.lb6 = ctk.CTkLabel(self.card_frame, text="Status :-", font=font_label)
        ...
        self.txt6.set(self.item[6].title() if str(self.item[6]).strip() else "Active")


        # Password
        self.lb5 = ctk.CTkLabel(self.card_frame, text="password :-", font=font_label)
        self.lb5.grid(row=5, column=0, sticky="w", padx=20, pady=8)
        self.txt5 = ctk.CTkEntry(self.card_frame, width=220, height=35, corner_radius=8, show="*")
    ...
    def modify(self):
        ...
        status = self.txt6.get().strip()
        password = self.txt5.get().strip()

        # Validation Checks
        if not name or not gender or not mobile or not email or not status or not password:
            ...
            q = "UPDATE students SET name=%s, gender=%s, mobile=%s, email=%s, password=%s, status=%s WHERE id=%s"
            self.cur.execute(q, (name, gender, mobile, email, password, status, admin_id))
            self.conn.commit()

            tkinter.messagebox.showinfo("Success", "Student Updated", parent=self.root1)
    ...
    def delete(self):
        ...
        if len(treeviewid) == 0:
            ...
            if tkinter.messagebox.askyesno("Confirm Delete", "Are you sure you want to delete this admin record?", parent=self.root):
                self.del_id = self.student_table.item(treeviewid).get("values")[0]
                q3 = "DELETE FROM students WHERE id=%s"
