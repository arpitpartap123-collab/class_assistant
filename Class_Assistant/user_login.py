...
class userLogin:
    ...
    def login(self):
        ...
        q = "SELECT id, status, name,email FROM students WHERE (email = %s OR mobile = %s) AND password = %s"
        ...
        if self.result:
            ...
            if status == "inactive":
                tkinter.messagebox.showwarning("Warning", "Student is inactive try again later", parent=self.root)
            else:
                tkinter.messagebox.showinfo("Success", "Student login successfully", parent=self.root)
                parent = self.root.master
                self.root.destroy()
                if parent and parent.winfo_exists():
                    parent.destroy()
                app = user_dashboard.StudentDashboard(self.result)
                app.root.mainloop()
        else:
