...
class Edit_Profile:
    ...
    def load_current_values(self):
        ...
        if not self.admin_id:
            return
        q = "SELECT name, mobile, email FROM admin WHERE id=%s"
        self.cur.execute(q, (self.admin_id,))
    ...
    def update(self):
        ...
        if not name or not mobile or not email:
            ...
            q = "UPDATE admin SET name=%s, mobile=%s, email=%s WHERE id=%s"
            self.cur.execute(q, (name, mobile, email, self.admin_id))
            self.conn.commit()
