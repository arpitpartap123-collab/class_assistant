...
from quizhistory import quizHistory  # Imported Quiz History component
...
class StudentDashboard:
    def __init__(self, student_detail=None):
        ...
        self.academicsMenu.add_command(label="Quiz History 📜", command=self.open_quiz_history)
        ...
        self.gradesMenu.add_command(label="View Quiz History", command=self.open_quiz_history)
        ...
        # Quiz History Sidebar Entry
        self.btn_history = ctk.CTkButton(
            self.sidebar_frame, text="📜  Quiz History", anchor="w",
            fg_color="transparent", text_color="#A0A5B5", hover_color="#202330", height=38,
            command=self.open_quiz_history
        )
        self.btn_history.grid(row=6, column=0, padx=15, pady=5, sticky="ew")
        ...
        self.btn_schedule.grid(row=7, column=0, padx=15, pady=5, sticky="ew")
        ...
        self.btn_grades.grid(row=8, column=0, padx=15, pady=5, sticky="ew")
        ...
        self.btn_profile.grid(row=9, column=0, padx=15, pady=5, sticky="ew")
    ...
    def build_middle_analytics_section(self):
        ...
        self.create_task_item(deadline_box, "Past Quiz Attempts", "View Results & Analytics", "#6366F1", action_cmd=self.open_quiz_history)
    ...
    def create_task_item(self, parent, title, date_str, tag_color, action_cmd=None):
        ...
        if action_cmd:
            btn_act = ctk.CTkButton(
                box, text="View", width=60, height=26,
                fg_color=tag_color, hover_color="#4F46E5",
    ...
    def open_quiz_history(self):
        """Launches the Quiz History Toplevel window attached to StudentDashboard parent."""
        student_id = self.get_student_id()
        if student_id is not None:
            quizHistory(student_id, parent=self.root)
        else:
            msg.showerror("Error", "Could not retrieve valid Student ID.")
